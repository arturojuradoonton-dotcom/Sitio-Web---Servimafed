...
const BASE_URL = "https://www.servimafed.com";
  ...
  return (
    <div className="flex items-center gap-3">
      {socials.map((s) => (
        <button
          key={s.name}
          onClick={() => handleShare(s.href)}
          aria-label={`Compartir en ${s.name}`}
          title={`Compartir en ${s.name}`}
          className={`w-10 h-10 border border-gray-200 rounded-full flex items-center justify-center text-gray-400 transition-all duration-300 cursor-pointer ${s.color}`}
        >
          {s.icon}
        </button>
      ))}
